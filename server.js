const express = require("express");
const fetch = require("node-fetch");
const fs = require("fs");
const path = require("path");

const app = express();
const PORT = process.env.PORT || 3000;
const BP_LIST_PATH = path.join(__dirname, "ranked.bplist");

// Fetch all ranked songs, paging through all pages without limit
async function fetchRankedMaps() {
  let page = 1;
  const songs = [];

  while (true) {
    const res = await fetch(`https://scoresaber.com/api/leaderboards?ranked=true&page=${page}`);
    if (!res.ok) {
      console.error(`Failed to fetch page ${page}: ${res.status} ${res.statusText}`);
      break;
    }
    const data = await res.json();

    if (!data.leaderboards || data.leaderboards.length === 0) {
      break; // no more pages
    }

    songs.push(
      ...data.leaderboards.map(lb => ({
        hash: lb.songHash.toUpperCase(),
        songName: lb.songName,
        difficulties: [] // empty, add if you want later
      }))
    );

    page++;
  }

  return songs;
}

// Generate the .bplist JSON content
function generateBplist(songs) {
  return JSON.stringify({
    playlistTitle: "ScoreSaber Ranked",
    playlistAuthor: "EvanBlokEnder",
    customData: {
      icon: "data:image/png;base64,"
    },
    songs
  }, null, 2);
}

// Update the ranked.bplist file
async function updateBplist() {
  try {
    console.log("Starting ranked.bplist update...");
    const songs = await fetchRankedMaps();
    const bplistContent = generateBplist(songs);
    fs.writeFileSync(BP_LIST_PATH, bplistContent);
    console.log(`Updated ranked.bplist with ${songs.length} songs.`);
  } catch (err) {
    console.error("Failed to update ranked.bplist:", err);
  }
}

// Route to download ranked.bplist file
app.get("/ranked.bplist", (req, res) => {
  if (fs.existsSync(BP_LIST_PATH)) {
    res.download(BP_LIST_PATH);
  } else {
    res.status(404).send("ranked.bplist not found, try again later.");
  }
});

// Start server and schedule updates every 6 hours
app.listen(PORT, () => {
  console.log(`Server running on port ${PORT}`);
  updateBplist(); // initial update
  setInterval(updateBplist, 6 * 60 * 60 * 1000); // every 6 hours
});
